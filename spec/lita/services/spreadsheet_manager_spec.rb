...
context 'on a new empty worksheet' do
  ...
  describe '#read' do
    it 'returns a worksheet instance' do
      allow(spreadsheetManager).to receive(:read).with('WS_SHEET').and_return(worksheet_mock)
      expect(spreadsheetManager.read('WS_SHEET')).to eq(worksheet_mock)
    end
  end
end
